...
import { Block, BlockPartialWithRelations, Form, BlockWithPartialRelations, FormPartialWithRelations, OptionsBlock, OptionsBlockPartialRelations } from "@/prisma/generated/zod";
...
export default function JDMACustomForm({ form }: JDMACustumFormProps) {
    ...
    const displayBlock = (block: BlockPartialWithRelations) => {
        switch (block.type_bloc) {
            case 'heading_1': {
                return (
                    <h1 key={block.id} className={cx(fr.cx('fr-mb-6w'))}>{block.content}</h1>
                )
            }
            case 'heading_2': {
                return (
                    <h2 key={block.id} className={fr.cx('fr-mb-6w')}>{block.content}</h2>
                )
            }
            case 'heading_3': {
                return (
                    <h3 key={block.id} className={fr.cx('fr-mb-6w')}>{block.content}</h3>
                )
            }
            case 'paragraph': { 
                return (
                    <p key={block.id} className={fr.cx('fr-mb-6w')}>{block.content}</p>
                )
            }
            case 'input_text': {
                return (
