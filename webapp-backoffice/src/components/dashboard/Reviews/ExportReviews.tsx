...
const ExportReviews = (props: Props) => {
    ...
    const [memoryKey, setMemoryKey] = React.useState<string>(generateRandomString(10))
    ...
    const applyChoice = (choice: 'all' | 'filtered') => {
        ...
        exportData.mutate(choice === 'all' ? {memoryKey} : {
            product_id,
            startDate,
            endDate,
	...
	return (
		<> 
            ...
            {exportStatus === 'inProgress' &&
                <div>
                    <div>
                        <p className={cx(classes.loading)}>Export en cours : {Math.round(progress * 100 )/100}%</p>
                    </div>
                    <div>
                        <div className={cx(classes.progressBarWrapper)}>
                            <div style={{width: `${progress}%`}} className={cx(classes.progressBar)}>
                                {" "}
                            </div>
                        </div>
                    </div>
