			...
			<ul className={fr.cx('fr-mt-4v')}>
				<li>
					<a
						...
						href={`${process.env.NEXT_PUBLIC_FORM_APP_URL}/Demarches/${form.form_template.slug !== 'root' ? `avis/${form.id}` : form.product_id}?preview=true&formConfig=${encodeURIComponent(JSON.stringify(zeroVersionFormConfig))}`}
						target="_blank"
				...
				{formConfigsToDisplay.map((formConfig, index) => (
					<li key={formConfig.id}>
						<a
							className={fr.cx('fr-link')}
							href={`${process.env.NEXT_PUBLIC_FORM_APP_URL}/Demarches/${form.form_template.slug !== 'root' ? `avis/${form.id}` : form.product_id}?preview=true&formConfig=${encodeURIComponent(JSON.stringify({ ...formConfig, displays: formConfig.form_config_displays, labels: formConfig.form_config_labels }))}`}
							target="_blank"
