...
const PublicStats = ({
	...
	useEffect(() => {
		if (forms.length > 0 && formId === 0) {
			setFormId(forms[0].id);
			setTmpFormId(forms[0].id);
		}
	}, [forms]);
	...
	return (
		<>
			<div className={fr.cx('fr-mt-5w')}>
				<h1 className={fr.cx(subtitle ? 'fr-mb-1v' : undefined)}>
					{product.title}
				</h1>
				{subtitle && <p className={fr.cx('fr-text--lead')}>{subtitle}</p>}
			</div>
...
const useStyles = tss.withName({ PublicStats }).create({
	applyContainer: {
