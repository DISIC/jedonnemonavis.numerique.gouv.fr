...
const BarChart = ({ data }: { data: PieData[]; kind: 'pie' }) => {
	const renderCustomizedLabel = ({
		...
		return (
			<g width={30}>
				<path
					d={svgPathSmileys[name as 'happy' | 'neutral' | 'bad']} // 24 x 24
					transform={`translate(${x - (24 * 1.35) / 2}, ${
						y - (24 * 1.35) / 2
					}),scale(1.35)`}
					fill="#ffffff"
					opacity={0.9}
