...
// memory forever. Consumers (queue.ts, workers, producers) degrade gracefully on null.
const redisClientSingleton = (): IORedis | null => {
	const url = process.env.REDIS_URL;
	if (!url) {
		console.warn(
			'[redis] REDIS_URL is not set — Redis-backed features (queues/workers) are disabled.'
		);
		return null;
	}
